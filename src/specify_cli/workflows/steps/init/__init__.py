...
class InitStep(StepBase):
    ...
    ``--ignore-agent-tools`` so local agent CLIs are not required and resolves
    the integration from the step config, then the workflow-level default
    integration.
    ...
    ``force``
        ...
        empty.
    ``ignore_agent_tools``
        Do not require the coding agent CLI to be installed.
        Defaults to ``true`` for unattended workflow runs.
    ``preset``
    ...
    def execute(self, config: dict[str, Any], context: StepContext) -> StepResult:
        ...
        # Workflows run unattended; local agent CLIs are not required by default.
