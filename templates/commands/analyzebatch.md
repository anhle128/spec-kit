...
handoffs:
  - label: Implement Tasks
    ...
    prompt: Run /speckit-implement to begin executing tasks.
scripts:
...
Note: This analysis workflow is expected to run AFTER `__SPECKIT_COMMAND_TASKS__` has produced a complete `tasks.md`, and BEFORE `__SPECKIT_COMMAND_IMPLEMENT__`. If the user explicitly states they are skipping analysis, you may proceed, but must warn that downstream rework risk increases.
...
Run `{SCRIPT}` from repo root **once** (combined `--json --require-tasks --include-tasks` mode / `-Json -RequireTasks -IncludeTasks`). Parse minimal JSON payload fields:
...
If any required artifact is missing, abort and instruct the user to run the missing prerequisite (`__SPECKIT_COMMAND_SPECIFY__`, `__SPECKIT_COMMAND_PLAN__`, or `__SPECKIT_COMMAND_TASKS__`).
...
Free-form focus context (after stripping flags): {ARGS}
